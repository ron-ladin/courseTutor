...
def test_imports():
    ...
    from travel_agent import data_client
    from travel_agent import planner
    from travel_agent import agent
    from travel_agent import mock_server
...
def test_models():
    ...
    try:
        bad_req = TravelRequest(
            destination="Tokyo",
            departure_date=date(2025, 6, 1),
            return_date=date(2025, 6, 1),  # Same as departure
            budget=5000,
            travel_style=["luxury"],
        )
...
def test_data_client_instantiation():
    ...
    # With custom URL
    client2 = DataClient(base_url="http://example.com")
