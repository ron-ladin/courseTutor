...
- **MockServer**: A local FastAPI server (`mock_server.py`) running on `localhost:8000` that exposes `/flights/search`, `/hotels/search`, and `/activities/search` endpoints, mimicking the Amadeus and Booking.com API styles.
- **DataClient**: An HTTP client (`data_client.py`) that calls the MockServer and returns typed Pydantic model instances to the PlanningLoop.
...
9. IF the MockServer is not running when the application starts, THE Agent SHALL display an error message instructing the user to start the server with `uvicorn mock_server:app`.
