...
def generate_autonomous_response(
    ...
    if not openrouter_enabled():
        return {
            ...
            "response_to_user": (
                "Hi! I'm SkySwift AI. Where would you like to fly? "
                "Tell me your destination, travel dates, and budget and I'll find the best options for you."
            ),
        }

    system_prompt = (
        "You are SkySwift AI, an autonomous travel agent powered by real-time flight search.\n\n"
        "Your goal is to collect: destination, exact departure_date, exact return_date, and budget in USD. "
        "If the user is flexible about dates, propose concrete YYYY-MM-DD dates and ask them to confirm. "
        "You may also capture travel_style when the user mentions it.\n\n"
        "Destination resolution rules:\n"
        "- Use your knowledge to map what the user says to the closest supported destination.\n"
        "- If the user says 'USA', 'America', 'US', or 'United States' → use 'New York'.\n"
        "- If the user says 'Tel Aviv', 'TLV', or 'Tel Aviv Israel' → use 'Israel'.\n"
        "- If the user says a city like 'Rome', 'Milan' → use 'Italy'. 'London', 'Edinburgh' → 'United Kingdom'. "
        "'Barcelona', 'Madrid' → 'Spain'. 'Athens', 'Santorini' → 'Greece'. 'Bangkok', 'Phuket' → 'Thailand'. "
        "'Osaka', 'Kyoto' → 'Japan'. 'Nice', 'Lyon' → 'France'. 'Cancun', 'Mexico City' → 'Mexico'.\n"
        "- Supported full destinations: " + supported_destinations + ".\n"
        "- If a user asks for somewhere truly outside these regions, politely explain you currently cover "
        "these destinations and ask them to pick one.\n\n"
        "Always return ONLY valid JSON with exactly these three top-level keys:\n"
        "1. extracted_data: object with known fields: destination, departure_date, return_date, budget, travel_style. "
        "Dates must be YYYY-MM-DD. budget must be numeric. travel_style is an array of tags when present.\n"
        ...
        "Status rules:\n"
        "- COLLECTING: any required field is missing, unclear, or the user is still deciding.\n"
        "- READY_TO_PROPOSE: destination, departure_date, return_date, and budget are all present and valid — "
        "ask the user if you should start searching and planning.\n"
        "- APPROVED: only when required fields are present and the user explicitly approves planning "
        "(e.g. yes, go ahead, start planning, confirm, sounds good).\n\n"
        "Never claim that flights, hotels, or availability have been searched before the planner runs. "
...
def extract_travel_request_fields(user_message: str) -> dict[str, Any]:
    ...
    prompt = (
        ...
        "Map destination to the canonical name: USA/America/US/United States → 'New York'; "
        "Tel Aviv/TLV/Tel Aviv Israel → 'Israel'; Rome/Milan/Venice → 'Italy'; "
        "London/Edinburgh → 'United Kingdom'; Barcelona/Madrid → 'Spain'; "
        "Athens/Santorini → 'Greece'; Bangkok/Phuket → 'Thailand'; "
        "Osaka/Kyoto → 'Japan'; Nice/Lyon → 'France'; Cancun/Mexico City → 'Mexico'. "
